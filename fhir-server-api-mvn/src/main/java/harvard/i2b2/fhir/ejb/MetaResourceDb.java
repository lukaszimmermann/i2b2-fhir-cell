...
public class MetaResourceDb {
	...
	public String addMetaResource(MetaResource p, Class c){
		...
		try{
			System.out.println("EJB Put MetaResource:"+c.cast(p.getResource()).getClass().getSimpleName());
			System.out.println("EJB MetaResources size:"+metaResources.size());
		}catch(Exception e){
			e.printStackTrace();
			return null;
		}
		...
		if (p.getMetaData().getId()==null) {
			p.getResource().setId(Integer.toString(getMetaResourceTypeCount(c)));
		...
		if (presentRes!=null) {
				...
				System.out.println("replacing resource with id:"+p.getMetaData().getId());
				metaResources.remove(presentRes);	
		}
		metaResources.add(p);
		
		System.out.println("EJB resources (after adding) size:"+metaResources.size());
	...
	public MetaResource getMetaResource(String id, Class c){
		...
		for(MetaResource p:metaResources){
			...
			System.out.println("examining resource with id:<"+p.getMetaData().getId() +"> for match to qid:<"+id+">");
			if (p.getMetaData().getId().equals(id)) {
				System.out.println("matched resource with id:"+p.getMetaData().getId());
				return p;
			}
		}
		return null;
	...
	public int getMetaResourceTypeCount( Class c){
		...
		for(MetaResource p:metaResources){
			if(!c.isInstance(p.getResource())) continue;
			count++;
		}
		return count;
	}
	...
	public MetaResource getParticularResource(Class c, String id) {
		for(MetaResource r:metaResources){
			  if(c.isInstance(r.getResource()) && r.getMetaData().getId().equals(id)){
				  return r;			  }
		  }
		return null;
	}
